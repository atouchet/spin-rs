...
- `portable-atomic` enables usage of the `portable-atomic` crate to support
  ...
  `portable-atomic` crate must also be set by the final binary crate. See the
  documentation for the `portable-atomic` crate for more information with some
  requirements for no-std build:
...
of 1.60.0 and above. This version will not be changed without a minor version
bump.
